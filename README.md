...
1）站点维护中添加/修改mt相关设置，浏览器仿真设置为否，站点地址设置到mt主域名(例如：https://ab.c-defg.hi/)。  
2）mt现在的登录控制比较严格，cookie可能频繁失效，推荐设置UA为常用浏览器UA（未验证是否有效）。  
3）mt现有适配的功能：站点测试，资源搜索、下载，签到（未验证是否可避免mt的不活跃账号检测）。  
4）mt站的RSS订阅、刷流、数据统计无效，订阅搜索有效（官方限制为最快6小时一次）。  
5）使用前需手动在mt站创建ApiKey。更换ApiKey后需在站点测试中重新选中mt测试，否则搜索和下载都会失败。  
6）常用功能已测试无异常，但不保证其他人的使用效果。
